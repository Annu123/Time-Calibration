...
///////////////////////////////////////////////////////////////////
   getStatistics().createHistogram(
		  new TH2F("TDiff_AB_vs_ID_thr1", 
			  "", 411, -20.5, 20.5,
			  200, 0 , 200 ) 
    );

    getStatistics().createHistogram(
		    new TH2F("TDiff_AB_vs_ID_thr2",
			    "", 411, -20.5, 20.5, 
			    200, 0 , 200 ) 
    );

    getStatistics().createHistogram(
		    new TH2F("TDiff_AB_vs_ID_thr3", 
			    "", 411, -20.5, 20.5,
			    200, 0 , 200 )
   );

    getStatistics().createHistogram
	    ( new TH2F("TDiff_AB_vs_ID_thr4", "",
		       411, -20.5, 20.5,
		       200, 0 , 200 )
   );
...
bool EventCategorizer::exec()
{
 ...
 /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////                
		getStatistics().getHisto2D("TDiff_AB_vs_ID_thr1")->GetXaxis()->SetTitle("Time difference AB for threshold 1 [ns]");
		getStatistics().getHisto2D("TDiff_AB_vs_ID_thr2")->GetXaxis()->SetTitle("Time difference AB for threshold 2 [ns]");
	        getStatistics().getHisto2D("TDiff_AB_vs_ID_thr3")->GetXaxis()->SetTitle("Time difference AB for threshold 3 [ns]");
	        getStatistics().getHisto2D("TDiff_AB_vs_ID_thr4")->GetXaxis()->SetTitle("Time difference AB for threshold 4 [ns]");
                getStatistics().getHisto2D("TDiff_AB_vs_ID_thr1")->GetYaxis()->SetTitle("ID of the scintillator");
	        getStatistics().getHisto2D("TDiff_AB_vs_ID_thr2")->GetYaxis()->SetTitle("ID of the scintillator");
	        getStatistics().getHisto2D("TDiff_AB_vs_ID_thr3")->GetYaxis()->SetTitle("ID of the scintillator");
	        getStatistics().getHisto2D("TDiff_AB_vs_ID_thr4")->GetYaxis()->SetTitle("ID of the scintillator");
...
{
	 double TDiff_AB = 0.;
         std::map<int, double> lead_timesA, lead_timesB;
         ...
         for (unsigned i = 1; i <= Hit.getSignalA().getRecoSignal().getRawSignal().getNumberOfPoints(JPetSigCh::Leading) && i <= Hit.getSignalB().getRecoSignal().getRawSignal().getNumberOfPoints(JPetSigCh::Leading);i++)	
			 { 
		    auto leadSearchA = lead_timesA.find(i);
		    auto leadSearchB = lead_timesB.find(i);
	 if (leadSearchA != lead_timesA.end() && leadSearchB != lead_timesB.end())
			 {

	      TDiff_AB = (leadSearchB->second - leadSearchA->second)/1000;
	      switch(i)
	 {
	      case 1:
	      getStatistics().getHisto2D("TDiff_AB_vs_ID_thr1")->Fill( TDiff_AB, Hit.getScintillator().getID() );										        break;
	      case 2:	
	      getStatistics().getHisto2D("TDiff_AB_vs_ID_thr2")->Fill( TDiff_AB, Hit.getScintillator().getID() );		                 
	      break;                          
	      case 3:			
	      getStatistics().getHisto2D("TDiff_AB_vs_ID_thr3")->Fill( TDiff_AB, Hit.getScintillator().getID() );														
	      break;                          
	      case 4:
	      getStatistics().getHisto2D("TDiff_AB_vs_ID_thr4")->Fill( TDiff_AB, Hit.getScintillator().getID() );							
	      break;                       
	 }
